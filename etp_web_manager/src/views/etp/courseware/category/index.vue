...
<script>
import {
  getCoursewareTreeselect, getCoursewareCategory, updateCoursewareCategory, addCoursewareCategory, delCoursewareCategory
  ...
  methods:{
    ...
    getTreeselect(){
      getCoursewareTreeselect().then(res => {
        ...
        this.categoryOptions.unshift({id:0,label:"全部课件"})
      })
    },
    addRoot(){
      this.reset()
    ...
    editCategory(){
      this.reset()
      let data = this.currentData
      getCoursewareCategory(data.id).then(res=>{
        this.form = res.data
      })
      this.title="修改分类"
      this.open = true
        ...
        if (valid) {
          this.$confirm('请确认是否保存当前分类信息?', '提示', {
            ...
            if (this.form.catId != undefined) {
              ...
              updateCoursewareCategory(this.form).then(response => {
                this.msgSuccess("修改成功");
                this.open = false;
                this.getTreeselect();
              });
            } else {
              addCoursewareCategory(this.form).then(response => {
    ...
    remove(){
      let data = this.currentData
        this.$confirm('是否确认名称为【'+data.label+'】的分类?', "警告", {
          ...
          return delCoursewareCategory(data.id);
        }).then(response => {
          this.getTreeselect();
