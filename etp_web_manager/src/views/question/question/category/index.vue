<template>
  <div class="container category-container">
    ...
    <el-button type="primary" style="width: 100%;margin-bottom: 10px" @click="addRoot()">增加根级题库</el-button>
...
<script>
import {addCategory,updateCategory,getTreeselect,getCategory,delCategory} from "@/api/category/category"
  ...
  data() {
    return{
      ...
      rules: {
        ...
        parentId: [
          { required: true, message: '请选择上级题库', trigger: 'blur' },
        ],
      },
      // =====右键菜单相关
      menuVisible: false,
  ...
  methods:{
    ...
    getTreeselect(){
      getTreeselect().then(res => {
        ...
        this.categoryOptions.unshift({id:0,label:"全部题目"})
      })
    },
    addRoot(){
      this.reset()
    ...
    remove(){
      let data = this.currentData
        this.$confirm('是否确认名称为【'+data.label+'】的题库?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
