...
<script>
import {
  updateTrainCategory, delTrainCategory, addTrainCategory, getTrainCategory, getTrainTreeselect
...
export default {
  ...
  methods:{
    ...
    editCategory(){
      this.reset()
      let data = this.currentData
      getTrainCategory(data.id).then(res=>{
        this.form = res.data
      })
      this.title="修改分类"
      this.open = true
        ...
        if (valid) {
          this.$confirm('请确认是否保存当前分类信息?', '提示', {
            ...
            if (this.form.catId != undefined) {
              ...
              updateTrainCategory(this.form).then(response => {
                this.msgSuccess("修改成功");
                this.open = false;
                this.getTreeselect();
              });
            } else {
              addTrainCategory(this.form).then(response => {
    ...
    remove(){
      let data = this.currentData
        this.$confirm('是否确认名称为【'+data.label+'】的分类?', "警告", {
          ...
          return delTrainCategory(data.id);
        }).then(response => {
          this.getTreeselect();
