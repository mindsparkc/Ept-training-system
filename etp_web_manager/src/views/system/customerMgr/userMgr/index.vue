  ...
  <div class="app-container">
    ...
    <el-dialog :title="title"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="选择用户组" prop="" label-width="85px">
              <el-select v-model="form.groupId" placeholder="选择用户组别" style="width: 100%;">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="角色">
              <el-select v-model="form.roleIds" multiple placeholder="请选择角色">
                <el-option
                  v-for="dict in customerRolesOptions"
...
export default {
  ...
  created() {
    ...
    // 获取客户角色
    this.getDicts("sys_customer_roles").then(response => {
      this.customerRolesOptions = response.data;
    });
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUser({...this.addDateRange(this.queryParams, this.dateRange), rootNodeType: 'tenant'}).then(response => {
    ...
    getTreeselect() {
      listDept({rootNodeType: 'tenant'}).then(response => {
        for (let i = 0; i < response.data.length; i++) {
          response.data[i].label = response.data[i].deptName;
          response.data[i].id = response.data[i].deptId;
        }
    ...
    handleQuery() {
      ...
      if (this.deptOptions && this.deptOptions.length > 0 && (this.queryParams.deptId == null || this.queryParams.deptId == undefined)) {
        this.queryParams.deptId = this.deptOptions[0].deptId;
      }
      this.getList();
    },
    ...
    handleAdd() {
      ...
      getUser().then(response => {
        ...
        /*let qyid = this.$store.getters.qyid;
        this.form.deptId = qyid;*/
      });
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
