      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="scope.row.deptId != topDeptId"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDept({...this.queryParams, rootNodeType: 'tenant'}).then(response => {
        this.deptList = this.handleTree(response.data, "deptId");
        if (this.deptList.length > 0) {
          this.topDeptId = this.deptList[0].deptId
        }
        this.loading = false;
      });
    },
    ...
    handleUpdate(row) {
      ...
      // 也就是说自己不能选择自己
      listDept({rootNodeType: 'tenant', excludeDeptId: row.deptId,}).then(response => {
        this.deptOptions = this.handleTree(response.data, "deptId");
      });
    },
    /** 提交按钮 */
