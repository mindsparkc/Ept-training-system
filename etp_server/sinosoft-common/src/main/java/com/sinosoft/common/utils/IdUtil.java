...
import cn.jkingtools.utils.HashUtil;
...
public final class IdUtil {
    ...
    public void init() {
        ...
        this.luaApplyHash = redisCache.loadScriptFromClassPath("/redis/sharding-id-apply.lua");
        this.luaRenewHash = redisCache.loadScriptFromClassPath("/redis/sharding-id-renew.lua");
        this.clientId = UUID.randomUUID().toString();
        int ret = redisCache.executeScriptReturnInt(this.luaApplyHash,
                "31".getBytes(), this.clientId.getBytes(), "600".getBytes()).intValue();
        if (ret < 0) {
    ...
    public void scheduledRenew() {
        try {
            int ret = redisCache.executeScriptReturnInt(this.luaRenewHash,
                    String.valueOf(shardingId).getBytes(), this.clientId.getBytes(), "600".getBytes()).intValue();
            if (ret < 0) {
