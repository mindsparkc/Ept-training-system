package com.sinosoft.etp.cp.tag.service.impl;
...
import com.sinosoft.etp.cp.tag.mapper.TTagWithCpMapper;
 ...
 */
@Service
public class TTagWithCpServiceImpl implements ITTagWithCpService {
    @Autowired
    private TTagWithCpMapper tTagWithCpMapper;
    ...
    {
        return tTagWithCpMapper.selectTTagById(id);
    }
    ...
    {
        return tTagWithCpMapper.selectTTagList(tTag);
    }
    ...
    public int insertTTag(TTag tTag)
    {
        ...
        return tTagWithCpMapper.insertTTag(tTag);
    }
    ...
    public int updateTTag(TTag tTag)
    {
        ...
        return tTagWithCpMapper.updateTTag(tTag);
    }
    ...
    {
        return tTagWithCpMapper.deleteTTagByIds(ids);
    }
    ...
    {
        return tTagWithCpMapper.deleteTTagById(id);
    }
    ...
    public int commpanyInsertTag(String tagStr) {
       ...
       return tTagWithCpMapper.commpanyInsertTag(list);
    }
    ...
    public int insertRelateTag(Long modelId,String tagStr,String modelType) {
        String[] tags = tagStr.split(",");
        //获取插入的标签 id
        List<Integer> tagIds = tTagWithCpMapper.getTagIdsByName(tags);
        ...
        int i = tTagWithCpMapper.insertRelateTag(paramMap);
        if(i>0){
            //新增成功，更新redis
    ...
    public int deleteRelateTag(Long modelId,String modelType) {
        //根据modelType 获取表名和关联字段
        if (StringUtils.isBlank(modelType) || modelId == null) {
            return 0;
        }
        Map<String,String> map =  TableFiledEnum.tableFiledMap(modelType);
        String tableName = map.get("tableName");
        String filed = map.get("filed");
        int i = tTagWithCpMapper.deleteRelateTag(modelId,tableName,filed);
        if(i>0){
            //删除成功，更新redis
            List<TTag> tagsDatas = getCompanyModelTags(modelType);
            TagsUtils.setTagsCache(modelType,tagsDatas);
    ...
    public List<TTag> getCompanyModelTags(String modelType){
        ...
        return tTagWithCpMapper.listRelateTag(tableName);
    }
}
