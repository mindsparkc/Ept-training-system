 ...
 */
@RestController
@RequestMapping("/admin/v1/train/category")
public class TrainCategoryController extends BaseController
{
    ...
    private static CategoryType.CategoryEnum categoryEnum = CategoryType.CategoryEnum.TRAIN_CATEGORY;


    @PreAuthorize("@ss.hasPermi('train:category:list')")
...
//        vos.add(0, vo);
     ...
     * 获取题目分类详细信息
     */
    @PreAuthorize("@ss.hasPermi('train:category:query')")
     ...
     * 新增题目分类
     */
    @PreAuthorize("@ss.hasPermi('train:category:add')")
    @Log(title = "习题集分类", businessType = BusinessType.INSERT)
    @PostMapping
     ...
     * 修改题目分类
     */
    @PreAuthorize("@ss.hasPermi('train:category:edit')")
    @Log(title = "习题集分类", businessType = BusinessType.UPDATE)
    @PutMapping
     ...
     * 删除题目分类
     */
    @PreAuthorize("@ss.hasPermi('train:category:remove')")
    @Log(title = "习题集分类", businessType = BusinessType.DELETE)
    ...
    public AjaxResult remove(@PathVariable Long[] catIds) {
        ...
        if (objectInterCategoryService.hasObj(categoryEnum, Arrays.asList(catIds))) {
            throw new RuntimeException("该分类下存在习题集，不允许删除！");
        }
