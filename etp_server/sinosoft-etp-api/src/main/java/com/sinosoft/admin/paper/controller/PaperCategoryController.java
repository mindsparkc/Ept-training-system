 ...
 */
@RestController
@RequestMapping("/admin/v1/paper/category")
public class PaperCategoryController extends BaseController
{
    @Resource
...
//    }
    ...
    @PreAuthorize("@ss.hasPermi('paper:category:list')")
...
//        vos.add(0, vo);
     ...
     * 获取试卷目分类详细信息
     */
    @PreAuthorize("@ss.hasPermi('paper:category:query')")
     ...
     * 新增试卷分类
     */
    @PreAuthorize("@ss.hasPermi('paper:category:add')")
     ...
     * 修改试卷分类
     */
    @PreAuthorize("@ss.hasPermi('paper:category:edit')")
     ...
     * 删除试卷分类
     */
    @PreAuthorize("@ss.hasPermi('paper:category:remove')")
    ...
    public AjaxResult remove(@PathVariable Long[] catIds) {
        ...
        if (objectInterCategoryService.hasObj(categoryEnum, Arrays.asList(catIds))) {
            throw new RuntimeException("该分类下存在试卷，不允许删除！");
        }
