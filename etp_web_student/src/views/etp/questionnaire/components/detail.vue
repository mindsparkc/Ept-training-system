  ...
  <div class="app-container">
    <div class="paper pre-pad">
      <div style="margin-left: 20px;padding:0 30px 30px">
        ...
        <div id="dx" style="padding-top: 10px;pointer-events: none">
          <div class="con-title">
...
<!--          <div style="text-align: center"><h1> {{ questionForm.wjmc }}</h1></div>-->
          <div v-if="questionForm.wjnr && questionForm.wjnr.length > 0"
               v-for="(item,index) in questionForm.wjnr">
  ...
  methods:{
    ...
    getQuestion(id){
      getQuestionnaire(id).then(res => {
        ...
        if(res.data.wjnr ==null){
          this.questionForm.wjnr = res.data.nr
        }

        if(this.questionForm.wjnr){
          this.questionForm.wjnr = JSON.parse(this.questionForm.wjnr)
        }
        //复选题答案回显
          this.questionForm.wjnr.forEach((item,index) => {
            if(item.type=='1'){
