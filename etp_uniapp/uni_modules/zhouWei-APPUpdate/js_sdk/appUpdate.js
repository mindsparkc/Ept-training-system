// #ifdef APP-PLUS 
import componentConfig from "@/common/componentConfig"
const platform = uni.getSystemInfoSync().platform;
// 主颜色
const $mainColor = componentConfig.appUpdateColor ? componentConfig.appUpdateColor : "0D3CD4";
// 弹窗图标url
...
const getDownload = function(data) {
	...
	dtask = plus.downloader.createDownload(data.download, {
	...
	// } 
	// else if(data.updateType == "silent"){//静默更新情况暂不考虑
	// 	dtask = plus.downloader.createDownload(data.download, {
	// 		filename: "_doc/update/"
	// 	}, function(download, status) {
	// 		if (status == 200) {
...
function updatePopup(data, callback) {
	...
	// 描述的列表
	const descriptionList = drawtext(data.versionDescribe, viewContentWidth);
	// 弹窗容器高度
...
export default function(isPrompt = false) {
	getCurrentNo(versionInfo => {
		componentConfig.getServerNo(versionInfo, isPrompt, res => {
			...
			// console.log("getServerNo>>>>>>>>>>>>", JSON.stringify(res));
			//判断是否需要更新 比较大小

			if (versionInfo.versionCode >= res.versionCode) {
				if (isPrompt) {
					uni.showToast({
						title: '当前已是最新版本',
						icon: 'none'
				...
				updatePopup(res, function() {
					if (/\.wgt$/i.test(res.download)) {
						getDownload(res);
					} else if (/\.html$/i.test(res.download)) {
