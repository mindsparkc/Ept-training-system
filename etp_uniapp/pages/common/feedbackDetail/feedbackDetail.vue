<template>
	<view>
		<view class="white">
			<view class="text-black">{{feedback.yjms}}</view>
			<view class="img-row">
				<view v-for="(item,index) in images" :key="index" @click="catImage(index)">

					<view style="margin-right: 20rpx; margin-top: 20rpx;">
						<u--image :showLoading="true" :src="item.url" width="200rpx" height="200rpx" 
							mode="aspectFill" radius="20rpx"></u--image>
					</view>
				</view>
			</view>
		...
		<view class="box" v-if="feedback.hf">
			<view class="text-black">客服回复：</view>
			<view class="text-grey">{{feedback.hf}}</view>
		...
		<view class="box" v-if="feedback.hf&&isResolve&&isShow">
			<view class="text-black">客服回复：</view>
...
<script>
	export default {
		...
		methods: {
			getFeedbackDetail() {
				uni.$u.api.getFeedback(this.feedbackId).then(res => {
					this.feedback = res.data
					this.images = this.parseData(this.feedback.fj)
			...
			catImage(index) {
				uni.navigateTo({
					url: '/pages/common/showImage/showImage?imgs=' + JSON.stringify(this.images) + '&current=' +
						index,
