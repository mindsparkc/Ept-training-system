<!-- 课程习题专用答题组件 -->
<template>


	<view class="page" v-if="data">
		<view>
			<!-- <text class="text-tag">{{data.lxbq}}题</text> -->
			<!-- <text class="text-question">{{data.tg}}</text> -->
			<!-- <view style="margin-top: 10px;">
				<mp-html :content="data.tg"></mp-html>
			</view> -->

			<mp-html :content="addQuestionType(data.lxbq+'题',data.tg)"></mp-html>

			<!-- <text class="text-score">({{data.score}}分)</text> -->
		</view>
		<!-- <view v-if="!confirmAnswer"> -->
		<view v-if="data.lx=='4'||data.lx=='5'">
			<view class="text-answer-tip">请在下方输入框中输入您的答案</view>
			<view v-for="(item,index) in completionValues" :key="index" v-if="data.lx=='4'">
				<view class="flex-row">
					<text class="text-no" v-if="completionValues!=null&&completionValues.length>1">{{index+1}}.
					</text>
					<u--input :placeholder="setCompletionTips(index)" border="surround" v-model="item.value" clearable>
					</u--input>
				</view>

			</view>

			<view v-if="data.lx=='5'">

				<kt-voice-textarea @input="input"></kt-voice-textarea>
			</view>

		</view>
		<!-- </view> -->
		<view v-for="(item,index) in options" :key="index" class="row" @click="checkChange(index)"
			v-if="data.lx=='1'||data.lx=='2'||data.lx=='3'">

			<view :class="[optionIsChecked(item.key)?'bg-selected-answer':'bg-normal-answer']">
				<text :class="[optionIsChecked(item.key)?'option-yes':'option-unchecked']">{{item.key}}</text>
				<text style="flex: 1;" class="text-question">{{item.value}}</text>
			</view>
		</view>

		<view class="bg-answer" v-if="confirmAnswer||hasUserAnswer">
			<view>
				<text class="text-tips">答案</text>
				<text class="text-tips-yes"
					v-if="data.lx=='1'||data.lx=='2'||data.lx=='3'||data.lx=='5'">{{tfAnswer(data.da)}}</text>
				<!-- 填空题答案 -->
				<view v-else v-for="(answerItem,answerIndex) in JSON.parse(data.da)" :key="answerIndex"
					class="text-input">
					<view class="flex-row">
						<text style="color: #909299; font-size: 14px;">【{{answerIndex+1}}】</text>
						<view v-for="(answerDataItem,answerDataIndex) in answerItem" :key="answerDataIndex">
							<text
								class="text-tips-yes">{{answerDataItem}}{{answerDataIndex==answerItem.length-1?"":"/"}}</text>
						</view>
					</view>
				</view>

			</view>
			<view style="margin-top: 15px;" v-if="data.lx=='1'||data.lx=='2'||data.lx=='3'">
				<text class="text-tips">您选择</text>
				<text
					:class="[checkAnswer()==tfAnswer(data.da)? 'text-tips-yes':'text-tips-no']">{{checkAnswer()}}</text>
			</view>
			<view style="margin-top: 15px;" v-if="data.lx=='5'">
				<text class="text-tips">您的回答</text>
				<text class="text-tips-yes">{{value}}</text>
			</view>
			<view style="margin-top: 15px;" v-if="data.lx=='4'">
				<text class="text-tips">您的回答</text>
				<view v-for="(item,index) in completionValues" :key="index" class="text-input">
					<text style="color: #909299; font-size: 14px;">【{{index+1}}】<text
							class="text-tips-yes">{{item.value?item.value:'没有作答'}}</text></text>
				</view>
			</view>
		</view>

		<view v-if="needAnalysis">
			<view class="row" style="margin-bottom: 10px;">
				<view class="vertical-line"></view>
				<text class="text-tips" style="margin-left:10px ;">题目解析</text>
			</view>

			<!-- <text style="margin-left: 15px;">{{data.jx?data.jx:"暂无解析"}}</text> -->
			<view style="margin-left: 15rpx;">
				<mp-html :content="data.jx?data.jx:'暂无解析'"></mp-html>
			</view>
		</view>
		<!-- <view v-if="!confirmAnswer"> -->
		<view class="btn-margin" v-if="data.lx=='2'||data.lx=='4'||data.lx=='5'">
			<u-button type="primary" shape="circle" :text="btnText" @click="confirm()"></u-button>
			<!-- <text class="btn-tips">做完本题记得点击确认答案按钮</text> -->

		</view>
		<!-- </view> -->

	</view>

</template>

<script>
	export default {
		name: "item-exercises",
		props: {

			//试题
			data: {
				type: Object,
				default: function() {
					return null;
				}
			},

			lessonId: {
				type: String,
				default: function() {
					return null;
				}
			}



		},
		mounted() {


		},
		watch: {
			data: {
				handler() {
					this.getQuestionUserAnswer();

					//模式  类型 1-单选 ，2-复选，3-判断（即只有两个选项 正确 错误的选择题  判断题答案  0：错  1：对），4-填空，5-问答
					if (this.data.lx == "3") { //处理判断题选项

						this.options = [{
								key: "A",
								value: "正确",
								isChecked: false,
							},
							{
								key: "B",
								value: "错误",
								isChecked: false,
							}
						];
						//判断题答案 0：错误  1：正确   正确1 错误2
						if (this.data.da) {
							if (this.data.da == "0") {
								this.options[1].isAnswer = true;
							} else {
								this.options[0].isAnswer = true;
							}
						}


					} else if (this.data.lx == "4") { //填空题
						if (this.completionValues == null || this.completionValues.length <= 0) {
							let gaps = 1;
							if (this.data.place) {
								gaps = this.data.place;
							}
							for (let i = 0; i < gaps; i++) {
								let item = {
									value: ""
								}
								this.completionValues.push(item);
							}
						}



					} else {
						if (this.options == null || this.options.length <= 0) {
							if (this.data.xx) {
								this.options = this.stringToArray(this.data.xx);
								for (let i = 0; i < this.options.length; i++) {
									this.options[i].isChecked = false;
									this.options[i].isAnswer = false;
									if (this.data.da) {
										if (this.options[i].key == this.data.da || (this.data.da.indexOf(this.options[i]
												.key)) != -1) {
											this.options[i].isAnswer = true;
										}
									}
								}
							}
						}



					}
				},
				deep: true,
				immediate: true

			}
		},
		data() {
			return {
				//  多选题 填空题 问答题出现确认答案按钮 
				checkedIndex: [], //被选中的选项index
				confirmAnswer: false, //是否确认答案  即选择结束显示答案解析 当前暂不隐藏输入框和确认按钮 
				options: [], //选项
				btnText: "确认",
				value: '', //输入的答案
				answer: [], //本题选择的答案
				completionValues: [], //填空题答案
				completionTips: "",
				needAnalysis: false, //显示解析
				userAnswer: null, //学员答案
				hasUserAnswer: false, //是否查询到学员答案


			};
		},
		methods: {
			//问答题输入框
			input(value) {
				this.value = value;
			},


			//判断选项是否被选中
			optionIsChecked(key) {
				if (this.answer && this.answer.length > 0) {
					if (this.answer.indexOf(key) == -1) { //没找到
						return false;
					} else {
						return true;
					}
				} else {
					return false;
				}


			},

			parseData() {
				//模式  类型 1-单选 ，2-复选，3-判断（即只有两个选项 正确 错误的选择题  判断题答案  0：错  1：对），4-填空，5-问答
				if (this.data.lx == "3") { //处理判断题选项

					this.options = [{
							key: "A",
							value: "正确",
							isChecked: false,
						},
						{
							key: "B",
							value: "错误",
							isChecked: false,
						}
					];
					//判断题答案 0：错误  1：正确   正确1 错误2
					if (this.data.da) {
						if (this.data.da == "0") {
							this.options[1].isAnswer = true;
						} else {
							this.options[0].isAnswer = true;
						}
					}


				} else if (this.data.lx == "4") { //填空题
					if (this.completionValues == null || this.completionValues.length <= 0) {
						let gaps = 1;
						if (this.data.place) {
							gaps = this.data.place;
						}
						for (let i = 0; i < gaps; i++) {
							let item = {
								value: ""
							}
							this.completionValues.push(item);
						}
					}



				} else {
					if (this.options == null || this.options.length <= 0) {
						if (this.data.xx) {
							this.options = this.stringToArray(this.data.xx);
							for (let i = 0; i < this.options.length; i++) {
								this.options[i].isChecked = false;
								this.options[i].isAnswer = false;
								if (this.data.da) {
									if (this.options[i].key == this.data.da || (this.data.da.indexOf(this.options[i]
											.key)) != -1) {
										this.options[i].isAnswer = true;
									}
								}
							}
						}
					}



				}
			},

			saveAnswers(da) {
				let that = this;
				that.needAnalysis = true;
				that.confirmAnswer = true;
				//同步缓存
				console.log("本次选择的答案是：", da);
				//保存答案
				let params = {
					stid: this.data.id,
					xyda: da
				}
				this.$emit("saveAnswer", params);


			},

			//点击选项 将被点击的选项从列表中移除或者加入并改变状态
			checkChange(index) {

				//判断是单选还是多选  若为单选 则取消其他选项  多选 则不变
				if (this.data.lx == '1' || this.data.lx == '3') { //单选或者判断题



					let da = ""; //用户选择的答案 其中判断题 选A代表0正确  B代表1错误

					for (let i = 0; i < this.options.length; i++) {
						this.options[i].isChecked = false; //单选题取消其他选项 
						if (index == i) {
							let option = this.options[i];
							option.isChecked = true;
							this.options.splice(i, 1, option);
							da = option.key

							this.answer = [];
							this.answer.push(da);
						}

					}
					if (this.data.lx == '3') { //判断题A正确-1  B错误-0
						if (da == 'A') { //正确
							da = '1'
						} else {
							da = '0'
						}
					}

					//单选或者判断 点击选项之后记录选项 
					this.saveAnswers(da);

				} else { //多选
					let muliOption = this.options[index];
					muliOption.isChecked = !muliOption.isChecked;
					this.options.splice(index, 1, muliOption);
					this.answer = [];
					for (let j = 0; j < this.options.length; j++) {
						if (this.options[j].isChecked) {
							this.answer.push(this.options[j].key);
						}
					}


				}
				console.log("用户选择的答案", JSON.stringify(this.answer));

			},

			setCompletionTips(index) {
				let kong = this.completionValues.length;
				if (kong != null && kong == 1) {
					return "请输入您的答案";
				} else {
					let num = index + 1
					return "请输入第" + num + "个空的答案";
				}


			},
			//用户选择的答案 主要用于处理多选题
			checkAnswer() {
				let answer = "";
				for (let i = 0; i < this.options.length; i++) {
					if (this.options[i].isChecked) {
						answer = answer.concat(this.options[i].key + ",")
					}

				}
				answer = answer.substr(0, answer.length - 1);

				return answer;
			},

			//确认答案
			confirm() {
				let isCheckedAnswer = false;

				let da = ""; //用户作答答案
				if (this.data.lx == '2') { //若为复选题 则判断是否选择了答案

					let das = [];
					for (let i = 0; i < this.options.length; i++) {
						if (this.options[i].isChecked) {
							isCheckedAnswer = true;
							das.push(this.options[i].key)
						}

					}
					da = das.join(",")

				}
				//若为问答题 则判断是否输入了答案
				if (this.data.lx == '5') {
					if (this.value) {
						isCheckedAnswer = true;
						da = this.value;
					}
				}
				//若为填空题
				if (this.data.lx == '4') {
					let values = this.completionValues;
					let isAllComplete = true;

					let daStrArray = [];
					if (values && values.length > 0) {
						for (let i = 0; i < values.length; i++) {
							daStrArray.push(values[i].value);
							if (!values[i].value) {
								isAllComplete = false;
							}
						}
					} else {
						isAllComplete = false;
					}
					if (isAllComplete) {
						isCheckedAnswer = true;
						da = JSON.stringify(daStrArray);

					}


				}


				if (isCheckedAnswer) {
					this.saveAnswers(da);
				} else {
					uni.showToast({
						title: '请先作答',
						icon: 'none'
					});
				}

			},
			//处理判断题答案显示
			tfAnswer(str) {
				if (this.data.lx == '3') { //若为判断题
					if (str == "0") {
						return "B"
					} else if (str == "1") {
						return "A"
					}
				} else {
					return str;
				}

			},

			//json string转换 json array
			stringToArray(str) {
				return JSON.parse(str);
			},



			//获取用户答题信息
			getQuestionUserAnswer() {
				uni.$u.api.getUserSubjectInfo(this.lessonId, this.data.id).then(res => {

					if (res.data && res.data.xyda) {

						this.userAnswer = res.data.xyda;
						this.setUserAnswer(this.userAnswer);

					}

				}).catch(err => {
					console.log(err);
				})
			},
			//用户答题信息回显  1-单选 ，2-复选，3-判断（即只有两个选项 正确 错误的选择题  判断题答案  0：错  1：对），4-填空，5-问答
			setUserAnswer(userAnswer) {
				console.log("用户答案", userAnswer);
				this.needAnalysis = true; //显示解析
				this.hasUserAnswer = true;
				switch (this.data.lx) {
					case "1": //单选
						this.answer = [];
						this.answer.push(userAnswer);
						for (let i = 0; i < this.options.length; i++) {
							if (this.options[i].key == userAnswer || (userAnswer.indexOf(
									this
									.options[i]
									.key)) != -1) {
								this.options[i].isChecked = true;
							}
						}
						break;
					case "2": //复选题
						this.answer = [];
						this.answer = userAnswer.split(',');


						for (let i = 0; i < this.options.length; i++) {
							if (this.options[i].key == userAnswer || (userAnswer.indexOf(
									this
									.options[i]
									.key)) != -1) {
								this.options[i].isChecked = true;
							}
						}
						break;
					case "3": //判断题 √
						this.answer = [];

						if (userAnswer == "0") { //错误  即选择了b
							this.options[1].isChecked = true;
							this.options[0].isChecked = false;
							this.answer.push('B');
						} else {
							this.options[0].isChecked = true;
							this.options[1].isChecked = false;
							this.answer.push('A');
						}

						break;

					case "4": //填空题
						let answerList = JSON.parse(userAnswer);
						if (answerList && answerList.length > 0) {
							this.completionValues = [];
							for (let i = 0; i < answerList.length; i++) {
								let completionValue = {
									value: answerList[i]
								}
								this.completionValues.push(completionValue)

							}

						}
						console.log(">>>", JSON.stringify(this.completionValues));

						break;
					case "5": //问答题√
						this.value = userAnswer;
						break;

					default:
						break;
				}
			},

			addQuestionType(type, content) {

				let typeHtml =
					`<span style='font-size: 12px;color: #FFFFFF;background-color: #037EFF;padding: 6rpx 10rpx;border-top-left-radius: 10rpx;border-top-right-radius: 10rpx;border-bottom-right-radius: 10rpx;margin-right:10px;'>${type}</span>`;

				//判断题干最外层的标签是否为<p>标签 若为p标签 则将拼接的题目类型加到p标签内防止换行
				let mergeContent = "";
				if (content.substring(0, 3) == '<p>') {

					let innerContent = content.substring(content.indexOf('<p>') + 3, content.indexOf('</p>'));
					let mergeString = typeHtml.concat(innerContent);

					mergeContent = content.replace(innerContent, mergeString);
				} else {
					mergeContent = typeHtml.concat(content);
				}

				return mergeContent;

			}
		}
	}
</script>

<style lang="scss">
	.page-item {
		display: flex;
		flex-direction: column;
		height: 100%;

	}

	.count-box {
		display: flex;
		flex-direction: row;
		justify-content: flex-end;
		align-items: center;
		margin-right: 15px;
	}

	.text-current-count {
		font-size: 20px;
		color: #00c6c4;
		font-weight: bold;
	}

	.text-total-count {
		font-size: 15px;
		color: #999999;
		font-weight: bold;
	}

	.content {
		font-size: 40px;
		align-self: center;
		margin-top: 40px
	}

	.page {
		background-color: #FFFFFF;
		padding: 15px 15px;
		margin: 15px;
		box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);
		border-radius: 10px;
	}

	.bg-tag {
		background-color: $uni-color-primary;
		border-radius: 5px;
		padding: 3px 5px;

	}

	.bg-answer {
		background-color: rgba(3, 126, 255, 0.1);
		border-radius: 5px;
		padding: 10px 15px;
		margin-top: 24px;
	}

	.text-score {
		font-size: 14px;
		color: $uni-text-color-grey;
		margin-left: 5px;
	}

	.text-tag {
		font-size: 12px;
		color: #FFFFFF;
		background-color: $uni-color-primary;
		padding: 3px 5px;
		margin-right: 12px;
		border-top-left-radius: 5px;
		border-top-right-radius: 5px;
		border-bottom-right-radius: 5px;
	}

	.text-question {
		line-height: 1.5em;
		font-size: 16px;
		color: $uni-text-color;
	}

	.text-tips {
		font-weight: bold;
		font-size: 16px;
		color: $uni-text-color;

	}

	.text-tips-yes {
		font-weight: bold;
		font-size: 16px;
		color: $uni-color-primary;
		margin-left: 5px;
	}

	.text-tips-no {
		font-weight: bold;
		font-size: 16px;
		color: #E30C0C;
		margin-left: 10px;
	}


	.text-white {
		color: #FFFFFF;
		font-size: 16px;
	}

	.option-unchecked {
		display: flex;
		justify-content: center;
		align-items: center;
		width: 30px;
		height: 30px;
		border-radius: 50%;
		background-color: #FFFFFF;
		color: $uni-text-color;
		margin-right: 10px;
		font-size: 16px;
		box-shadow: 0px 1px 1px 1px rgba(0, 0, 0, 0.1);
	}

	.option-yes {
		display: flex;
		justify-content: center;
		align-items: center;
		width: 30px;
		height: 30px;
		border-radius: 50%;
		margin-right: 10px;
		background-color: $uni-color-primary;
		font-size: 16px;
		color: #FFFFFF;
		box-shadow: 0px 1px 1px 1px rgba(0, 0, 0, 0.1);
	}

	.option-no {
		display: flex;
		justify-content: center;
		align-items: center;
		width: 30px;
		height: 30px;
		border-radius: 50%;
		background-color: #E30C0C;
		font-size: 16px;
		color: #FFFFFF;
	}

	.bg-normal-answer {
		background-color: #FFFFFF;
		border-radius: 5px;
		display: flex;
		width: 100%;
		padding: 5px 10px;
		flex-direction: row;
		align-items: center;
		justify-content: flex-start;
	}

	.bg-selected-answer {
		width: 100%;
		padding: 5px 10px;
		background-color: rgba(3, 126, 255, 0.1);
		border-radius: 5px;
		display: flex;
		flex-direction: row;
		align-items: center;
		justify-content: flex-start;
	}

	.row {
		display: flex;
		flex-direction: row;
		align-items: center;
		justify-content: flex-start;
		margin-top: 12px;
	}

	.btn-margin {
		margin: 15px 50px 0px 50px;
		text-align: center;
	}

	.vertical-line {
		height: 13px;
		width: 3px;
		border-radius: 5px;
		background-color: $uni-color-primary;
	}

	.text-answer-tip {
		margin-top: 20px;
		margin-bottom: 10px;
		color: #606266;
		font-size: 14px;

	}

	.text-no {
		font-size: 18px;
		color: #909193;

	}

	.btn-tips {
		color: #F1A532;
		font-size: 10px;
	}

	.flex-row {
		display: flex;
		flex-direction: row;
		align-items: center;

		margin-top: 3px;
	}

	.bg-user-answer {
		background-color: rgba(3, 126, 255, 0.1);
		border-radius: 5px;
		margin-top: 16px;
		padding: 10px;
		font-weight: bold;
		font-size: 16px;
		color: $uni-text-color;
	}

	.text-input {

		vertical-align: middle;
		color: #303133;
		font-size: 14px;
	}

	.text-no {
		font-size: 14px;
		color: #909193;
		margin-right: 10px;

	}
</style>
